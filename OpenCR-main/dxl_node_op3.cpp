...
int buttonState = HIGH, buttonStateKill = HIGH; // initial state (HIGH = not pressed)
int lastButtonState = HIGH, lastButtonStateKill = HIGH;
unsigned long lastDebounceTime = 0, lastDebounceTimeKill = 0;
unsigned long debounceDelay = 50, debounceDelayKill = 50;
int countKill = 0, countStrategy = 0;
...
void read_button() {
  ...
  // Send strategyNumber when the button is pressed
  Serial.print("#");
  Serial.println(strategyNumber);
  ...
  // Send kill state when the kill button is pressed
  Serial.print("*");
  Serial.println(kill);
}
