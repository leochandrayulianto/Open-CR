...
bool dxl_debug_menu_shwo_ctrltbl()
{
  ...
  addr = (uint32_t)&p_dxl_mem->Strategy - (uint32_t)p_dxl_mem;
  DEBUG_SERIAL.print(addr);
  DEBUG_SERIAL.print("\t Strategy        \t "); DEBUG_SERIAL.print  (p_dxl_mem->Strategy);
  DEBUG_SERIAL.print("\t 0x");                  DEBUG_SERIAL.println(p_dxl_mem->Strategy,HEX);

  addr = (uint32_t)&p_dxl_mem->Kill - (uint32_t)p_dxl_mem;
  DEBUG_SERIAL.print(addr);
  DEBUG_SERIAL.print("\t Kill            \t "); DEBUG_SERIAL.print  (p_dxl_mem->Kill);
  DEBUG_SERIAL.print("\t 0x");                  DEBUG_SERIAL.println(p_dxl_mem->Kill,HEX);

  addr = (uint32_t)&p_dxl_mem->Roll_ - (uint32_t)p_dxl_mem;
  DEBUG_SERIAL.print(addr);
  DEBUG_SERIAL.print("\t Roll_GY25       \t "); DEBUG_SERIAL.print  (p_dxl_mem->Roll_);
  DEBUG_SERIAL.print("\t 0x");                  DEBUG_SERIAL.println(p_dxl_mem->Roll_,HEX);

  addr = (uint32_t)&p_dxl_mem->Roll_ - (uint32_t)p_dxl_mem;
  DEBUG_SERIAL.print(addr);
  DEBUG_SERIAL.print("\t Pitch_GY25      \t "); DEBUG_SERIAL.print  (p_dxl_mem->Pitch_);
  DEBUG_SERIAL.print("\t 0x");                  DEBUG_SERIAL.println(p_dxl_mem->Pitch_,HEX);

  addr = (uint32_t)&p_dxl_mem->Yaw_ - (uint32_t)p_dxl_mem;
  DEBUG_SERIAL.print(addr);
  DEBUG_SERIAL.print("\t Yaw_GY25        \t "); DEBUG_SERIAL.print  (p_dxl_mem->Yaw_);
  DEBUG_SERIAL.print("\t 0x");                  DEBUG_SERIAL.println(p_dxl_mem->Yaw_,HEX);
}
